...
| Wild creature movement/behavior | §18.5 | Pending | Wild Encounter scaffold (Wk 14-16) intentionally keeps wild creatures stationary — GDD §18.5 "Wild Creature Behavior" (habitat-based Signal pool selection, pack dynamics, capture difficulty scaling, Celestial behavior in Region 4) is itself still PENDING in the GDD, so movement/approach behavior isn't designed yet. Explicitly considered and rejected reusing an existing `CompanionAI` movement pattern (Orbit/HiddenShadow/Blink/etc.) as a shortcut — those were built for a bonded, friendly follower, and conflating "approaches the player" with "friendly" would guess at content §18.5 hasn't decided (wild behavior may need to vary by species/habitat, not share one rule). Address once §18.5 has real design content. |
...
| Map expansion (playable area size) | — | Pending | Current `SampleScene.unity` tilemap is a small 30×19 placeholder test room. Backlog: expand it using only the tile assets already in `Assets/Tiles` (`S_Ground_Placeholder`, `S_Wall_Placeholder`, `land_brown`, `23_0`/`24_0`/`34_0`/`35_0`/`36_0` — all already painted into the scene via `WorldPalette.prefab`) — confirmed with user, no new art/tileset import needed. Decided: build it chunked from the start via `WorldChunkManager` (it isn't chunked today — only the player is) rather than one giant unchunked map, matching CLAUDE.md's locked "World = GameObject Chunks" rule and avoiding a retrofit once Phase 4's Hub + Realms world streaming lands. |
| Enemy-side lane/position occupancy (7×5 grid, mirrored) | Combat_Directive §Part 3, Attack_Pattern_Directive §Part 8 | Deferred | The 2026-08-12 formation grid work built the 7×5 occupancy model (exclusive positions within a shared lane) for the **player side only**, deliberately deferred confirmed with the user during planning. `Attack_Pattern_Directive_v0_1_0.md`'s own 2026-08-12 errata says the enemy side should be "un-deferred," but no multi-enemy battle system exists yet to hang it on (only one enemy stage slot renders today) — the directive's abstract resolution outran what's actually buildable against the current encounter/rendering pipeline. Revisit once a specific skill or encounter genuinely needs 2+ simultaneous enemies; full gap breakdown (what's needed, in rough cheap-to-expensive order) in `DECISIONS.md` -> [Combat] "Enemy-side position support." |
