...
| Architecture_Directive | v0.1.0 | Docs/Architecture_Directive_v0_1_0.md | Full-codebase architecture review (UI Toolkit vs Scene rendering, assembly split, god-file decomposition), phased migration plan toward official Unity 2D patterns | 2026-08-25: Phase 1 BUILT AND COMPLETE, in 4 increments — `Phasix.World`/`Phasix.Player`/`Phasix.UI`/`Phasix.Bootstrap` all split out of `Phasix.Runtime` (`UI` via moving `HudTooltip`/`BattleSummaryController` into `Combat/`; `Bootstrap` via moving `GameManager` out of `Core/` plus simplifying `EventBus`'s battle-outcome events, whose `BattleResult` payload turned out to have zero real readers). The original 7-assembly proposal was disproven by a full cross-folder audit (`Core`/`Creatures`/`Combat` were one mutually-coupled cluster), but all of those cycles are now closed for real — see the doc's own errata history and `DECISIONS.md` → `[Architecture]`. Phase 3 and `VFX_Pipeline_Directive` are now genuinely unblocked. Part 1 still has the full current-state findings (strengths and debt) grounding the plan. 2026-08-26: Phase 3's first real slice BUILT — one player-side stage creature (slot 0) now renders as a real Scene `SpriteRenderer`, via a RenderTexture bridge (not direct camera compositing — this doc's own Part 3 investigation notes were wrong about that, and about "the frozen overworld stays visible"; see the doc's own 2026-08-26 errata and `DECISIONS.md` → `[Architecture]`). Beat Sequence lunge motion and hit-flash aren't mirrored onto the shadow yet — accepted, temporary, tracked as `KNOWN_ISSUES.md` → `[PRESENTATION-001]`. |
